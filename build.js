...
const css = () => new Promise((resolve, reject) => sass({file: `src/styles/entry.scss`}, (err, result) => err ? reject(err) : resolve(result)))
  .then(({css}) => purifycss(['src/components/**/*.js'], css.toString()))
...
const sw = () => swPrecache('build/public/sw.js', {
  ...
  dynamicUrlToDependencies: {
    '/': ['./src/index.html', './build/public/bundle.css', './build/public/bundle.js', './build/public/manifest.json', './package.json'] // bust cache when these change
  },
  skipWaiting: true,
...
const copy = () => fs.copyAsync(`src/static/`, `./build/public/`);
const html = () => fs.copyAsync(`src/index.html`, `./build/public/index.html`);
...
tasks
  ...
  .set('sw', sw)
  .set('build', () => run('clean')
    .then(() => Promise.all([run('client'), run('css'), run('copy'),run('html')]))
    .then(() => run('rev'))
    .then(() => run('sw'))
  );
